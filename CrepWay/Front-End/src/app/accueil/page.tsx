...
import PropositionStyles from '../styles/Proposition.module.css'
...
export default function Main() {
  ...
  const handleAddPanier = async (product: any) => {
    const commande = {
  ...
  return (
    <div>
      ...
      <motion.section className={HeroStyles.hero}>
        <div className={HeroStyles.container}>
          <div className={HeroStyles.hero_content}>
            ...
            >
              <h1>
                Bienvenue sur <span style={{ color: 'yellow' }}>Crep'way</span>
                <br /> Aulnay-sous-bois
              </h1>
              <p>
          ...
          >
            <div className={AboutStyles.aboutUsContent}>
              ...
              <p>
                ...
                appréciez CREP'WAY. Non seulement la crêpe est personnalisable, mais également votre dessert.
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <section id="recommandations" className={PropositionStyles.propositionsSection} ref={ref}>
        <h1>Nos Recommandations !</h1>
        <div className={PropositionStyles.cardContainer}>
          {propositions.map((proposition, index) => (
            <motion.div
              key={index}
              className={PropositionStyles.card}
              initial={{ opacity: 0, x: 100 }}
              animate={isPropositionsVisible ? { opacity: 1, x: 0 } : {}}
              transition={{ duration: 0, delay: isPropositionsVisible ? index * 0.1 : 0 }}
            >
              <img src={proposition.image} alt={proposition.title} />
              <div className={PropositionStyles.cardInfo}>
                <h3>{proposition.title}</h3>
                <div className={PropositionStyles.cardSubInfo}>
                  <p>{proposition.price.toFixed(2)} €</p>
                  <button onClick={() => handleAddPanier(proposition)}>Ajouter au panier</button>
                </div>
              </div>
            </motion.div>
          ))}
